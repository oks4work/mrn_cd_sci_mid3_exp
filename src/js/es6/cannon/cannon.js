...
class CannonLine {
    ...
    pinMaker(config) {
        let container, pin, circle, text;

        container = cannon.elements.pinContainer;

        pin = $ts.ce({tag: "li", parent: container});
        pin.style.top = `${config.top - 18}px`;

        circle = $ts.ce({tag: "div", class: "circle", parent: pin});
        circle.style.backgroundColor = cannon.config.pin.color[this.pinCount];

        text = $ts.ce({tag: "div", class: "text", parent: pin});
...
class Bar {
    ...
    initiate() {
        ...
        this.element.style.marginRight = `${cannon.config.graph.barMargin}px`;
